...
const TagTab: React.FC<TagTabProps> = ({ theme, daysCount }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error(error);
      alert(lang === 'ar' ? "حدث خطأ أثناء توليد العلامات." : "An error occurred while generating tags.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={`max-w-5xl mx-auto p-2 md:p-4 font-cairo ${isRtl ? 'text-right' : 'text-left'}`} dir={dir}>
      <div className="bg-white rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 shadow-xl border border-gray-100 mb-6 md:mb-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div className="flex flex-col">
            <h2 className="text-xl md:text-3xl font-black text-gray-900 flex items-center gap-3">
              <span>🏷️</span> {t('tag.title')}
            </h2>
            <p className="text-[10px] text-gray-400 font-bold mt-2 uppercase">SEO Ready - {daysCount} {t('app.days')}</p>
          </div>
          <select 
            value={country}
        ...
        <div className="space-y-6">
          <div className="w-full">
            <label className="block text-[10px] font-black text-gray-400 mb-2 uppercase">{lang === 'ar' ? 'الموضوع' : 'Topic'}</label>
            <input
              type="text"
              value={topic}
              onChange={(e) => setTopic(e.target.value)}
              placeholder={lang === 'ar' ? "عن ماذا يتحدث المحتوى الخاص بك؟" : "What is your content about?"}
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <label className="block text-[10px] font-black text-gray-400 mb-2 uppercase">{lang === 'ar' ? 'المنصة' : 'Platform'}</label>
              <select
                value={platform}
            ...
            <div className="flex-[2] flex flex-col">
              <label className="block text-[10px] font-black text-gray-400 mb-2 uppercase">{lang === 'ar' ? 'تأكيد' : 'Action'}</label>
              <button
                onClick={handleGenerate}
                disabled={loading}
                className={`${buttonClasses[theme]} w-full text-white px-6 md:px-10 py-4 md:py-5 rounded-[1.5rem] md:rounded-[2rem] transition-all font-black disabled:opacity-50 shadow-lg flex items-center justify-center gap-4 text-sm md:text-xl active:scale-95 cursor-pointer`}
              >
                {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <span>{lang === 'ar' ? 'توليد العلامات' : 'Generate Tags'}</span>}
              </button>
            </div>
          </div>
        </div>
      ...
      {Array.isArray(tags) && tags.length > 0 && (
        <div className="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3.5rem] shadow-sm border border-gray-100 animate-in slide-in-from-bottom-5">
          <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 border-b border-gray-50 pb-6">
            <h3 className="text-lg md:text-xl font-black text-gray-900">{lang === 'ar' ? '🎯 العلامات المقترحة' : '🎯 Suggested Tags'}</h3>
            <button
              onClick={copyToClipboard}
              className={`w-full md:w-auto px-8 py-3 rounded-xl border-2 transition-all font-black text-xs md:text-sm cursor-pointer ${copied ? 'bg-green-500 border-green-500 text-white' : 'bg-gray-50 border-gray-100 text-blue-600 hover:bg-gray-100'}`}
            >
