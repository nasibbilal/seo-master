...
const VideoAuditorTab: React.FC<VideoAuditorTabProps> = ({ theme }) => {
  ...
  const handleAudit = async () => {
    ...
    try {
      ...
      console.error(error);
      alert("حدث خطأ أثناء فحص الفيديو. يرجى التأكد من الرابط.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-6xl mx-auto p-4 font-cairo text-right">
      <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-xl border border-gray-100 mb-10">
        <h2 className="text-2xl md:text-3xl font-black text-gray-900 mb-8 flex items-center gap-4">
          ...
          محلل الفيديو الذكي (Video Auditor)
        </h2>
        
        <div className="space-y-6">
          <div className="w-full">
            <label className="block text-[10px] font-black text-gray-400 mb-2 mr-4 uppercase tracking-widest">رابط الفيديو أو وصف المحتوى</label>
            <textarea
              rows={3}
              value={videoInput}
              onChange={(e) => setVideoInput(e.target.value)}
              placeholder="ضع رابط يوتيوب/تيك توك أو اكتب تفاصيل الفيديو (عنوان، فكرة، سيناريو)..."
          ...
          <button
            onClick={handleAudit}
            disabled={loading}
            className={`${themeClasses} text-white w-full py-5 rounded-2xl font-black disabled:opacity-50 transition-all flex items-center justify-center gap-3 shadow-lg active:scale-95`}
          >
            {loading ? <div className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : "بدء الفحص المخبري للفيديو"}
          </button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-5 duration-700 pb-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
               <h3 className="font-black text-xl mb-6 flex items-center gap-2 border-b border-gray-50 pb-4">🚀 خطة التحسين الاستراتيجية</h3>
                  ...
                  {result.optimizationPlan?.map((step, idx) => (
                    <div key={idx} className="bg-gray-50 p-6 rounded-2xl border-r-4 border-blue-500 flex justify-between items-center group hover:bg-blue-50 transition-all">
                       <div className="flex-1">
                          <span className="text-[10px] font-black text-gray-400 uppercase tracking-tighter mb-1 block">الهدف: {step.target}</span>
                       ...
                       <div className={`px-4 py-2 rounded-xl text-[10px] font-black ${step.impact === 'High' ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'}`}>
                          تأثير: {step.impact === 'High' ? 'عالي' : 'متوسط'}
                       </div>
                    </div>
                  )) || <p className="text-gray-400 text-center">لا توجد توصيات</p>}
               </div>
            </div>

            <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
               <h3 className="font-black text-xl mb-6 flex items-center gap-2 border-b border-gray-50 pb-4">⚠️ نقاط الخلل الحرجة</h3>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  )) || <p className="text-gray-400 text-center col-span-2">لا توجد عيوب مكتشفة</p>}
               </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
               ...
               <div className="relative z-10 text-center">
                  ...
                  <h4 className="text-gray-400 font-black text-[10px] uppercase tracking-widest mb-2">مجموع نقاط الـ SEO</h4>
                  <div className="text-6xl font-black mb-6">{result.seoScore}<span className="text-xl text-gray-500">/100</span></div>
                  
                  <div className="space-y-6">
                    <div>
                      <div className="flex justify-between text-[10px] font-black uppercase mb-1">
                         <span>فرصة التفاعل</span>
                    ...
                    <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
                       <span className="text-[10px] font-black text-gray-500 uppercase block mb-1">توقع الاحتفاظ بالجمهور</span>
                       <p className="text-xs font-bold text-gray-300 italic">"{result.retentionEstimate || 'غير متوفر'}"</p>
                    </div>
                  </div>
               </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
               <h4 className="font-black text-sm mb-4">مطابقة معايير المنصات</h4>
               <div className="space-y-4">
