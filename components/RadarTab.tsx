...
const RadarTab: React.FC<{ theme: ThemeColor; daysCount: number; onTrendDetected?: (title: string) => void }> = ({ theme, daysCount, onTrendDetected }) => {
  ...
  const platformOptions = [
    { id: Platform.YOUTUBE, name: lang === 'ar' ? 'يوتيوب 🎥' : 'YouTube 🎥', icon: '🎥' },
    { id: Platform.TIKTOK, name: lang === 'ar' ? 'تيك توك 🎵' : 'TikTok 🎵', icon: '🎵' },
    { id: Platform.GOOGLE, name: lang === 'ar' ? 'جوجل 🔍' : 'Google 🔍', icon: '🔍' },
    { id: Platform.INSTAGRAM, name: lang === 'ar' ? 'إنستغرام 📸' : 'Instagram 📸', icon: '📸' },
    { id: Platform.FACEBOOK, name: lang === 'ar' ? 'فيسبوك 👥' : 'Facebook 👥', icon: '👥' },
    { id: Platform.PINTEREST, name: lang === 'ar' ? 'بينتريست 📌' : 'Pinterest 📌', icon: '📌' },
  ];

  const isRtl = dir === 'rtl';

  return (
    <div className={`max-w-7xl mx-auto p-2 md:p-4 font-cairo ${isRtl ? 'text-right' : 'text-left'}`} dir={dir}>
      <div className="bg-white rounded-2xl md:rounded-[3rem] p-6 md:p-12 shadow-xl border border-gray-100 mb-8 md:mb-10">
        <h2 className="text-xl md:text-3xl font-black mb-8 flex items-center gap-3">📡 {t('radar.title')}</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-8 md:mb-10">
          ...
          <div>
            <label className="text-[9px] font-black text-gray-400 mb-1.5 block uppercase tracking-widest">{t('modal.platform')}</label>
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            <label className="text-[9px] font-black text-gray-400 mb-1.5 block uppercase tracking-widest">{t('radar.region')}</label>
        ...
        <button 
          onClick={handleScan} 
          disabled={loading} 
          className="w-full px-10 py-4.5 rounded-xl md:rounded-[2rem] bg-slate-900 text-white font-black text-md md:text-xl shadow-2xl hover:bg-black active:scale-95 transition-all flex items-center justify-center gap-3 cursor-pointer"
        >
          {loading ? <div className="w-5 h-5 border-3 border-white/30 border-t-white rounded-full animate-spin" /> : <><span>🔥</span> {t('radar.startScan')}</>}
        </button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <div className="lg:col-span-2 space-y-6">
          <h3 className="text-lg font-black mb-4 flex items-center gap-2">{lang === 'ar' ? '📊 نتائج الرصد المباشر' : '📊 Live Monitoring Results'}</h3>
          {sorted && sorted.length > 0 ? (
                          ...
                          <tr>
                            <td colSpan={3} className="px-4 py-3 bg-blue-50/10">
                              <div className="p-5 rounded-2xl border border-blue-100 bg-white shadow-sm animate-in fade-in slide-in-from-top-1">
                                <div className="mb-3">
                                  <span className="text-[9px] font-black text-blue-600 uppercase mb-1 block">{lang === 'ar' ? 'العنوان الجاذب:' : 'Catchy Title:'}</span>
                                  <p className="text-xs font-black text-gray-900 leading-snug">{strategyResult[ins.id].title}</p>
                                </div>
                                <div>
                                  <span className="text-[9px] font-black text-purple-600 uppercase mb-1 block">{lang === 'ar' ? 'خطة التنفيذ:' : 'Execution Plan:'}</span>
            ...
            <div className="py-20 text-center bg-gray-50 rounded-2xl border-4 border-dashed border-gray-100 opacity-40">
              ...
              <p className="font-black text-gray-400 text-sm">{lang === 'ar' ? 'الرادار بانتظار مسح التوجهات الحالية..' : 'Radar waiting for trend scanning..'}</p>
            </div>
          )}
        </div>
        ...
        <div className="lg:sticky lg:top-8 h-fit">
           <h3 className="text-lg font-black mb-4 flex items-center gap-2">
             <span className="text-blue-600">⬛</span> {lang === 'ar' ? 'المربع الأسود (تحليل الفجوات)' : 'Blackbox (Gap Analysis)'}
           </h3>
              ...
              {gapAnalysis ? (
                <div className="animate-in slide-in-from-top-4 relative z-10 flex flex-col h-full">
                  <div className="flex items-center gap-3 mb-8">
                    ...
                    <div>
                      <h4 className="text-xl font-black leading-tight">{lang === 'ar' ? 'فجوة مكتشفة!' : 'Gap Discovered!'}</h4>
                  ...
                  <div className="space-y-6 flex-1">
                    <div className="bg-white/5 p-5 rounded-2xl border border-white/10">
                      <span className="text-[9px] font-black text-gray-500 uppercase block mb-3">{lang === 'ar' ? 'الكلمات المفتاحية للأهداف 🚀' : 'Target Keywords 🚀'}</span>
                    ...
                    <div className="space-y-4">
                      <div className="group">
                        <span className="text-[9px] font-black text-blue-500 uppercase block mb-2 mr-1">{lang === 'ar' ? 'العنوان المقترح للتصدر' : 'Suggested Ranking Title'}</span>
                        <div className="bg-white/5 p-4 rounded-xl border border-white/10 text-xs font-black text-gray-100 flex justify-between items-center group-hover:bg-white/10 transition-all">
                          ...
                          <button onClick={() => handleCopy(gapAnalysis.suggestedTitle, 'title')} className={`text-[8px] px-2 py-1.5 rounded-lg shrink-0 mx-2 transition-all cursor-pointer ${copiedTitle ? 'bg-green-600' : 'bg-white/10 hover:text-white'}`}>
                      ...
                      <div className="group">
                        <span className="text-[9px] font-black text-purple-500 uppercase block mb-2 mr-1">{lang === 'ar' ? 'وصف SEO الاستراتيجي' : 'Strategic SEO Description'}</span>
                        <div className="bg-white/5 p-4 rounded-xl border border-white/10 text-[10px] font-bold text-gray-400 leading-relaxed group-hover:bg-white/10 transition-all relative">
                          ...
                          <button onClick={() => handleCopy(gapAnalysis.suggestedDesc, 'desc')} className={`absolute bottom-3 left-3 text-[8px] px-2 py-1.5 rounded-lg transition-all cursor-pointer ${copiedDesc ? 'bg-green-600 text-white font-black' : 'bg-white/10 hover:text-white'}`}>
                            {copiedDesc ? (lang === 'ar' ? '✓ تم النسخ' : '✓ Copied') : (lang === 'ar' ? '📋 نسخ الوصف' : '📋 Copy Description')}
                          </button>
                        </div>
                      </div>
                    </div>
                ...
                <div className="h-full flex flex-col items-center justify-center opacity-25 text-center py-12">
                  ...
                  <p className="text-lg font-black">{lang === 'ar' ? 'بانتظار تحليل الرادار..' : 'Awaiting Radar analysis..'}</p>
                  <p className="text-[10px] font-bold mt-2">{lang === 'ar' ? 'سيتم جلب أفضل الكلمات لضرب النتائج فور الاكتشاف.' : 'Best keywords for top ranking will be displayed upon detection.'}</p>
                </div>
              )}
           </div>
        </div>
