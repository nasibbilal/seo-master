...
const App: React.FC = () => {
  ...
  const handlePreviewLogo = async () => {
    if (!newChannelIdentifier && !newChannelName) {
      alert("يرجى إدخال اسم القناة أو معرفها أولاً");
      return;
    }
    ...
    try {
      ...
      console.error(e);
      alert("تعذر جلب الشعار الحقيقي عبر API، سيتم استخدام شعار افتراضي.");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row-reverse font-cairo overflow-x-hidden" dir="rtl">
      <Sidebar 
        activeTab={activeTab} 
      ...
      <main className="flex-1 lg:mr-72 p-3 md:p-10 min-h-screen transition-all duration-300">
          ...
          <div className="flex items-center justify-between w-full lg:w-auto gap-3 sm:gap-4">
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <h1 className="text-lg sm:text-2xl font-black text-gray-900 tracking-tight">SEO Master 🚀</h1>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="lg:hidden relative" ref={dropdownRef}>
                ...
                {isChannelDropdownOpen && (
                  <div className="absolute top-full left-0 mt-3 w-48 bg-white border border-gray-100 rounded-xl shadow-2xl py-3 z-[1500] animate-in fade-in slide-in-from-top-2">
                    <div className="px-4 mb-2 text-[8px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50 pb-2">القنوات</div>
                    {channels.map(ch => (
                      <button key={ch.id} onClick={() => handleChannelSwitch(ch.id)} className={`w-full text-right px-4 py-2 text-[10px] font-bold transition-all flex items-center gap-3 ${activeChannelId === ch.id ? 'bg-blue-50 text-blue-600' : 'hover:bg-gray-50'}`}>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
            ...
            <div className="bg-slate-900 px-4 md:px-6 py-2.5 md:py-3 rounded-xl md:rounded-2xl flex flex-row items-center gap-4 shadow-lg w-full max-w-[340px] sm:w-auto sm:min-w-[280px] mx-auto lg:mx-0">
              <div className="flex justify-between items-center gap-2">
                <span className="text-[9px] font-black text-slate-400 uppercase whitespace-nowrap">المدى:</span>
                <span className="bg-blue-600/20 text-blue-400 px-2 py-0.5 rounded-lg text-[10px] font-black min-w-[40px] text-center">{daysCount}ي</span>
              </div>
              <input 
                type="range" 
                min="1" 
            ...
            <div className="hidden lg:flex items-center gap-2 w-full sm:w-auto">
              ...
              <div className="relative flex-1 sm:flex-none" ref={dropdownRef}>
                ...
                >
                  ...
                  <span className="truncate text-gray-900 leading-tight flex-1 text-right">{activeChannel?.name || 'اختر القناة'}</span>
                ...
                {isChannelDropdownOpen && (
                  <div className="absolute top-full right-0 mt-3 w-full bg-white border border-gray-100 rounded-2xl md:rounded-[2rem] shadow-2xl py-4 z-[1500] animate-in fade-in slide-in-from-top-2">
                    <div className="px-6 mb-2 text-[8px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50 pb-2">القنوات المربوطة</div>
                    <div className="max-h-[250px] overflow-y-auto">
                      {channels.map(ch => (
                        <button key={ch.id} onClick={() => handleChannelSwitch(ch.id)} className={`w-full text-right px-6 py-3.5 text-xs font-bold transition-all flex items-center gap-4 ${activeChannelId === ch.id ? 'bg-blue-50 text-blue-600' : 'hover:bg-gray-50'}`}>
                    ...
                    <button onClick={() => { setIsChannelDropdownOpen(false); setIsAddChannelModalOpen(true); }} className="w-full text-blue-600 text-[10px] font-black py-3 border-t border-gray-50 hover:bg-gray-50 mt-2 italic">+ إضافة قناة جديدة</button>
                  </div>
                )}
              </div>
            </div>
        ...
        {isAddChannelModalOpen && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[2000] flex items-center justify-center p-3 sm:p-4">
            <div className="bg-white w-full max-w-lg rounded-2xl sm:rounded-[3rem] p-6 sm:p-10 shadow-2xl border-4 border-blue-100 relative overflow-y-auto max-h-[90vh] animate-in zoom-in-95 duration-300">
               <button onClick={() => setIsAddChannelModalOpen(false)} className="absolute top-6 left-6 text-gray-400 hover:text-red-500 transition-colors text-xl font-black">✕</button>
               
               <div className="text-center mb-8">
                  <div className="w-16 h-16 bg-blue-50 rounded-2xl flex items-center justify-center text-3xl mx-auto mb-4">🛰️</div>
                  <h2 className="text-xl font-black text-gray-900">ربط قناة جديدة</h2>
                  <p className="text-[10px] font-bold text-gray-400 uppercase mt-1 tracking-widest">تنسيق البيانات الحقيقي عبر API</p>
               </div>

               <div className="space-y-5">
                  <div>
                    <label className="text-[9px] font-black text-gray-400 uppercase mr-3 mb-1.5 block">اسم القناة</label>
                    <input 
                      type="text" 
                      value={newChannelName} 
                      onChange={(e) => setNewChannelName(e.target.value)} 
                      placeholder="اسم القناة للتمييز" 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="text-[9px] font-black text-gray-400 uppercase mr-3 mb-1.5 block">المنصة</label>
                      <select 
                    ...
                    <div>
                      <label className="text-[9px] font-black text-gray-400 uppercase mr-3 mb-1.5 block">معرف القناة / Handle</label>
                      <input 
                        type="text" 
                  ...
                  {newChannelPlatform === Platform.YOUTUBE && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="text-[9px] font-black text-gray-400 uppercase mr-3 mb-1.5 block tracking-widest">YouTube Key 1</label>
                        <input 
                          type="password" 
                          value={tempKey} 
                      ...
                      <div>
                        <label className="text-[9px] font-black text-gray-400 uppercase mr-3 mb-1.5 block tracking-widest">YouTube Key 2</label>
                        <input 
                          type="password" 
                          value={tempKey2} 
                  ...
                  <div className="flex flex-col items-center gap-4 py-4 bg-blue-50/50 rounded-2xl border border-dashed border-blue-200">
                     {previewLogo ? (
                       <div className="flex flex-col items-center animate-in fade-in scale-in-95">
                         ...
                         <span className="text-[9px] font-black text-green-600 uppercase">تم تأكيد الشعار الحقيقي</span>
                       </div>
                     ) : (
                       <button 
                       ...
                       >
                         ...
                         <span className="text-blue-600 font-black text-[10px] underline">
                           {fetchingLogo ? "جاري الاتصال بقواعد البيانات..." : "اضغط هنا لجلب صورة القناة الحقيقية"}
                         </span>
                       </button>
                     )}
                  </div>
